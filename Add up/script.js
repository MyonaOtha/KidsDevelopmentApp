...
function gameOver() {
  if (game.filter((number) => number === null).length === 0) {
    const sameNeighbors = game.find((tile, i) => {
      const isRightSame =
        game[i + 1] && (i + 1) % 4 !== 0 ? tile.value === game[i + 1].value : false;
...
function generateNewNumber() {
  // 0.9 probability of 1, 0.1 probability of 2
  const p = Math.random() * 100;
  return p <= 90 ? 1 : 2;
}

function addRandomNumber() {
  // Adds either a 1 or a 2 to an empty position in the game array
  const emptyCells = game.map((_, index) => index).filter((index) => game[index] === null);
...
to the same value, combine the values together (e.g. 1+1 = 2)
*/
